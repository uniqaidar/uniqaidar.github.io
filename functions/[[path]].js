...
// Intercepts /sitemap.xml and /sitemap_index.xml for ALL request types
// (Googlebot, Bingbot, Google Search Console fetch tool, humans).
// All other requests pass through via context.next() — zero cost.
//
// WHY THIS FILE EXISTS:
//   functions/index.js    → maps to GET /  only
//   functions/sitemap.xml.js → shadowed when no static file exists; Cloudflare
//                              routes /sitemap.xml to the SPA catch-all instead
//   functions/[[path]].js → maps to EVERY path — guaranteed to run before the
//                           SPA's _redirects catch-all swallows the request
//
// PLACE THIS FILE AT: functions/[[path]].js  (double square brackets, exactly)
...
// ── Sitemap response headers — kills every cache layer ─────────────────────────
...
// ── Main handler — fires for every request ──────────────────────────────────────
