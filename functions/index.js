...
// ── Sitemap XML builder ───────────────────────────────────────────────────────
// Served directly from this function — bypasses CDN cache and the Cloudflare
// infrastructure 403 that blocks static file fetches on *.pages.dev.
// No separate function file needed — this file already intercepts all requests.
async function buildSitemapXml(context) {
    const BASE       = 'https://uniqaidar.pages.dev';
    const dateToday  = new Date().toISOString().slice(0, 10);

    let fonts = [];
    try {
        const txt = await fetchAsset(context, `${BASE}/fonts.json`);
        if (txt) { const p = JSON.parse(txt); if (Array.isArray(p)) fonts = p; }
    } catch (_) {}

    let catIds = [];
    try {
        const html = await fetchAsset(context, `${BASE}/`);
        if (html) {
    ...
    if (catIds.length > 0) {
        ...
        for (const cat of catIds) {
            lines.push('  <url>');
            lines.push(`    <loc>${esc(`${BASE}/?cat=${encodeURIComponent(cat)}`)}</loc>`);
    ...
    if (fonts.length > 0) {
        ...
        for (const font of fonts) {
            ...
            const loc = `${BASE}/?cat=${encodeURIComponent(firstCat)}&font=${encodeURIComponent(font.name)}`;
            lines.push('  <url>');
            lines.push(`    <loc>${esc(loc)}</loc>`);
            lines.push(`    <lastmod>${esc(font.dateAdded || dateToday)}</lastmod>`);
...
export async function onRequestGet(context) {
    ...
    // ── Sitemap intercept — runs for ALL requests (crawlers AND humans) ───────
    // This must be checked BEFORE the crawler guard below, so that Google Search
    // Console's fetch tool (which uses a browser-like UA) also gets the XML.
